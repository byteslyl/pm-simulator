...
|------|--------|--------|---------|---------|
| INT-01 意图分类结果 | 刘俊辰&吕彦蕾 · Agent | 路毅 · 关系引擎 | Agent → 规则引擎 | 函数调用 / 消息队列 |
| INT-02 信任档位 | 路毅 · 关系引擎 | 刘俊辰&吕彦蕾 · Agent | 规则引擎 → Agent | 函数返回 / 状态查询 |
| INT-03 事实披露记录 | 刘俊辰&吕彦蕾 · Agent | 路毅 · 评分引擎 | Agent → 规则引擎 | 事件日志 |
